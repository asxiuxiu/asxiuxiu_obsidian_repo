...
**写作策略工具箱**（按内容灵活选用）：Why 先行（引入新概念前先让读者感到"这问题确实需要解决"）、比喻贯穿（直觉先行）、因果推导（讲清演进链条）、场景绑定（绑定具体使用/故障场景）、问题链衔接（段落结尾自然引出下一问题）、用自己的话写（拒绝复制粘贴）、原子化（一篇一个核心观点）、渐进式摘要（按笔记目的分层压缩）、有效失败（记录曾经的误解）、表格只用于总结。
...
- 问题链完整：复杂知识点是否按"问题 → naive 方案 → 改进"演进？

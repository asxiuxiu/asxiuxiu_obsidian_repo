...
### 链接规范
- **完整路径**：跨目录 wikilink 必须从 vault root 写完整路径，如 `[[Notes/C++编程/xxx]]`，禁止省略 `Notes/` 前缀。
- **精确标题锚点**：heading 跳转尽量使用完整标题文本，如 `[[Note#完整标题文本|别名]]`，避免只用前缀。
- **表格内转义**：wikilink 在表格中必须转义管道符，格式为 `[[路径\|别名]]`。
- **禁止嵌套语法**：严禁把 wikilink 嵌套进 Markdown 链接 URL，即禁止 `[显示文本]([[路径|别名]])`。如需自定义显示文本，直接使用 wikilink 别名：`[[路径|显示文本]]`。
- **修改前置**：任何对 `.md` 笔记的修改操作（增删改链接、调整结构）**必须先读取 `obsidian-markdown` skill**。

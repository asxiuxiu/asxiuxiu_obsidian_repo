...
- **数据导向设计（DOD，Data-Oriented Design）的基础**：现代引擎常用的 **ECS（Entity-Component-System，实体-组件-系统）**架构和 **SoA（Structure of Arrays，数组结构）** 等设计，本质上都是为了配合缓存体系而生的。ECS 把同类型的数据（如所有物体的位置）紧凑地排在一起，而不是让每个物体携带自己的位置、速度、血量等混杂数据——这样 CPU 一次加载进缓存的 64 字节就全是"位置"，没有浪费。DOD 的核心理念是"以缓存友好的数据布局为首要目标"，而非面向对象设计中"以行为封装为中心"。
...
现代 CPU 的缓存不是"一块大缓存"，而是分成了好几层，每层速度和容量都不一样。你可以把它想象成一个**多层工具架**：

- **L1 缓存**是最贴近 CPU 的一层，速度极快（几个时钟周期就能访问），但容量极小（几十 KB）。为了同时满足"取指令"和"读数据"的需求，L1 通常被拆成两半：**L1i（Instruction Cache，指令缓存）**专门存即将执行的机器指令，**L1d（Data Cache，数据缓存）**专门存程序要读写的变量数据。两者并行工作，互不干扰。
- **L2 缓存**比 L1 慢一些，但容量更大（几百 KB 到 1MB），作为 L1 的"后备仓库"。
- **L3 缓存**（也叫 LLC，Last-Level Cache，末级缓存）容量最大（几 MB 到几十 MB），被所有核心共享，是 CPU 与主存之间的最后一道防线。
- **主存（RAM）**就是任务管理器里看到的"内存"，容量最大（几 GB 到几十 GB），但速度最慢，访问一次要等上百个时钟周期。
...
| 1990s | Pentium / Pentium Pro | **片内 L1（分离 I$ / D$）+ 片外 L2** | 流水线加深，取指和数据访问竞争同一缓存成为瓶颈。分离指令缓存（I-Cache，I$）和数据缓存（D-Cache，D$）解决结构冒险 |

> **符号注记**：`I$` 和 `D$` 中的 `$` 是计算机体系结构领域的传统简写，代表"Cache"（因为 `C` 容易和"Clock"混淆，而 `$` 形似缓存的"快速存储"含义）。`I$` = Instruction Cache，`D$` = Data Cache。
...
#### 深入：False Sharing 的微观过程

前面我们提到 False Sharing 会让性能暴跌，但它到底在硬件层面是怎么发生的？要回答这个问题，必须先理解缓存一致性协议——**MESI（Modified-Exclusive-Shared-Invalid）**。简单来说，MESI 是多核 CPU 用来保证"各个核心的缓存数据保持一致"的硬件协议，它以 **Cache Line 为单位**管理同步。理解了这一点，False Sharing 的灾难场景就清晰了。
...
| 2 | — | 写 `y`，发现为 I，发 **RFO**（Request For Ownership，请求所有权） | 转发 RFO 给 A |
...
理解了缓存的组织原理和优化手段后，让我们看看这些知识在游戏引擎中的具体落地点。
...
> [!info] 延伸阅读
> - [[Notes/操作系统/CPU与内存架构/内存对齐与填充|内存对齐与填充]] — 了解 Cache Line 边界与 False Sharing
> - [[Notes/操作系统/性能分析与工具/Cache-Miss分析实战|Cache-Miss分析实战]] — 用 perf 量化 Cache Miss

...
| 状态  | 笔记                                                        | 核心问题                                            | 引擎映射                        |
| :-: | :-------------------------------------------------------- | :---------------------------------------------- | :-------------------------- |
|  ✅  | [[构造函数与析构函数基础\|构造函数与析构函数基础]]                              | 构造/析构的职责、`new`/`delete` 与构造/析构的关系、含指针成员类的不变式       | 引擎资源句柄类的诞生与销毁语义          |
|  ⬜  | 构造函数进阶：默认、显式、委托与继承构造                                     | 构造函数的生成规则、explicit 的防御、`= default` / `= delete` | UE 宏生成的 explicit 构造函数       |
...
|  ✅  | [[Notes/C++编程/构造、析构与拷贝控制/移动语义后的对象状态约定\|移动语义后的对象状态约定]]（Day 4 前置） | 移动构造后源对象的状态、合法操作集合、可析构保证                        | 移动后对象的置空策略                  |
...
|  ✅  | [[union 与类型安全\|union 与类型安全]]                                    | union 的内存共享机制、类型标签、placement new 与显式析构、`std::variant` 的类型安全封装 | 引擎事件系统、类型安全多态负载 |
...
| ✅ | [[返回值优化与 guaranteed elision\|返回值优化与 guaranteed elision]]（Day 26 前置） | RVO/NRVO 的编译器机制、C++17 强制省略（prvalue 语义改革） | 函数返回 `Mat4`/`Quat` 时的性能预期 |
...
| 状态 | 笔记 | 核心问题 | 引擎映射 |
|:---:|:---|:---|:---|
| ✅ | [[完美转发\|完美转发]] | `std::forward` 的必要性、与 `std::move` 的本质区别、万能引用、引用折叠 | 引擎 `Array::emplace`、工厂函数 |
...
| ✅ | [[智能指针与所有权模型\|智能指针与所有权模型]] | `unique_ptr`/`shared_ptr`/`weak_ptr` 的边界、循环引用、`make_shared`/`make_unique` | UE `TSharedPtr`/`TWeakObjectPtr`、SelfGameEngine 所有权策略 |
...
| ✅ | [[auto 类型推导详解\|auto 类型推导详解]] | `auto` 的三种推导形式、`auto` 与 `auto&` 与 `auto&&` 的区别 | 现代 C++ 代码中的变量声明策略 |
| ⬜ | RTTI 的实现成本与替代方案 | `typeid`/`dynamic_cast` 的底层、禁用 RTTI 后的做法 | 引擎自建类型注册表、反射系统 |
| ✅ | [[SFINAE：替换失败不是错误\|SFINAE：替换失败不是错误]] | 编译期条件分支、`enable_if` 的实现原理、SFINAE-friendly 设计 | 容器对 `trivially_copyable` 的特化 |
| ✅ | [[C++20 Concepts 与约束式泛型\|C++20 Concepts 与约束式泛型]] | `concept`/`requires` 的声明式约束、与 SFINAE 的对比 | 引擎接口的 concept 化表达 |
...
| 状态 | 笔记 | 核心问题 | 引擎映射 |
|:---:|:---|:---|:---|
| ✅ | [[类模板基础与模板参数推导\|类模板基础与模板参数推导]] | 类模板语法、类型参数与非类型参数、模板参数推导、模板代码为什么放头文件 | 引擎泛型容器 `Array<T, N>` 的设计基础 |
...
| 状态 | 笔记 | 核心问题 | 引擎映射 |
|:---:|:---|:---|:---|
| ✅ | [[运算符重载的原则与陷阱\|运算符重载的原则与陷阱]] | 哪些运算符该重载、对称性、自赋值安全、与内置语义的一致性 | 引擎数学类型的运算符设计（`Vec3`、`Mat4`） |
| ⬜ | 自增自减的前置与后置 | `operator++()` vs `operator++(int)` 的语义差异、效率差异 | 迭代器的前置/后置自增实现 |
| ⬜ | 类型转换运算符 | `operator T()` 的隐式转换风险、`explicit` 转换运算符（C++11） | 引擎中的显式类型转换设计 |
| ✅ | [[Lambda 表达式与闭包\|Lambda 表达式与闭包]] | 捕获模式（值/引用/初始化捕获）、闭包类型、泛型 lambda（C++14） | 引擎中回调与异步操作的 lambda 使用 |
| ⬜ | Lambda 捕获的悬垂风险 | 引用捕获栈变量、this 捕获的陷阱、移动捕获（C++14 初始化捕获） | 帧回调中的 lambda 生命周期管理 |
| ✅ | [[std-function 的实现与开销\|std::function 的实现与开销]] | 类型擦除、小对象优化（SOO）、虚函数调用开销、与函数指针的对比 | 引擎委托系统的轻量级替代设计 |
| ✅ | [[观察者模式与委托\|观察者模式与委托]] | 观察者模式解耦、Subject/Observer 生命周期、Delegate 与回调区别 | UE `DECLARE_DELEGATE`、SelfGameEngine 信号系统 |
...
| 状态 | 笔记 | 核心问题 | 引擎映射 |
|:---:|:---|:---|:---|
| ✅ | [[C++11 内存模型与 happens-before\|C++11 内存模型与 happens-before]] | 顺序一致性、数据竞争的定义、同步关系、sequenced-before | 跨线程组件读写、主线程回调投递 |
| ✅ | [[原子操作与无锁编程基础\|原子操作与无锁编程基础]] | `std::atomic` 的特化、CAS 循环、ABA 问题、LL/SC 架构差异 | 任务队列的 lock-free 实现 |
| ✅ | [[内存序：relaxed、acquire、release、seq_cst\|内存序：relaxed、acquire、release、seq_cst]] | 四种内存序的具体语义、选错的后果、memory fence | 并行 System 的读写屏障、命令缓冲提交 |
| ✅ | [[条件变量与虚假唤醒\|条件变量与虚假唤醒]] | 等待-通知机制、谓词等待的必要性、`wait_for` 的精度问题 | 线程池的任务等待与唤醒 |
| ✅ | [[thread_local 的实现与边界\|thread_local 的实现与边界]] | 线程局部存储的存储位置、生命周期、平台差异、动态加载问题 | 每线程帧分配器、随机数生成器状态 |
| ✅ | [[工作窃取队列与线程池设计\|工作窃取队列与线程池设计]] | 任务分解、双端队列窃取、负载均衡、任务亲和性 | SelfGameEngine 线程池核心架构 |
| ✅ | [[异步编程模型与 future-promise\|异步编程模型与 future/promise]] | 异步结果传递、`std::async` 的实现方式（launch policy）、packaged_task | 异步加载资源的回调封装 |
...
| 状态 | 笔记 | 核心问题 | 引擎映射 |
|:---:|:---|:---|:---|
| ✅ | [[vector 的扩容策略与摊还分析\|std::vector 的扩容策略与摊还分析]] | 增长因子的选择、reallocate 时的元素迁移、强异常安全 | 引擎 `Array` 的增长因子与内存预留 |
| ✅ | [[unordered_map 的底层原理与性能陷阱\|unordered_map 的原理与性能陷阱]] | 桶链结构、rehash 代价、cache miss、开放寻址法替代 | 引擎 `HashMap`、SparseSet 的设计动机 |
| ✅ | [[std-function 的实现与开销\|std::function 的实现与开销]] | 类型擦除、小对象优化、虚函数调用开销 | 引擎委托系统的轻量级替代设计 |
...
| ✅ | [[SoA、AoS 与 AOSOA\|SoA、AoS 与 AOSOA]] | 三种数据布局的内存访问模式、SIMD 友好性对比、切换代价 | ECS 组件存储、粒子系统数据布局 |
...
| 笔记 | 对应练习 | 说明 |
|:---|:---|:---|
| [[构造函数与析构函数基础\|构造函数与析构函数基础]] | Day 1：`String` 构造/析构 | 构造函数申请资源、析构函数释放资源、`new[]`/`delete[]` 配对 |
| [[对象内存布局：从 struct 到 class\|对象内存布局]] | Day 1 补充阅读：`String` 的 `char*` / `size_t` 排布 | 理解对象本身内存与堆上字符数组的关系 |
| [[对象生存期与 RAII\|对象生存期与 RAII]] | Day 2：`String` 边界处理 | 从所有权角度理解析构时为什么 `delete[]` |
| [[类模板基础与模板参数推导\|类模板基础与模板参数推导]] | Day 5：`Array<T, N>` | 类模板语法、非类型参数、模板定义放头文件 |
| [[explicit 关键字\|explicit 与隐式转换]] | Day 5 补充阅读：`Array<T, N>` | 防止模板类的意外隐式转换 |
| [[原始内存操作与对象生命周期的边界\|原始内存操作]] | Day 6：`UniquePtr` / Day 9：`SharedPtr` | `placement new`、控制块内存布局 |
| [[智能指针与所有权模型\|智能指针与所有权模型]] | Day 9：`SharedPtr` / Day 10：`MakeShared` | 共享所有权、引用计数、控制块 |
| [[完美转发\|完美转发]] | Day 10：`MakeShared` / Day 38：`emplace_back` | 原地构造、万能引用、std::forward |
| [[值类别与移动语义\|值类别与移动语义]] | Day 3：`String` 拷贝构造/赋值 | 深拷贝 vs 移动的本质区别 |
| [[noexcept 关键字\|noexcept 与异常规格]] | Day 4：`String` 移动构造/赋值 | 移动语义优化的前提 |
| [[union 与类型安全\|union 与类型安全]] | Day 13：`Variant` | union 内存共享、类型标签、placement new |
| [[SFINAE：替换失败不是错误\|SFINAE 与 type_traits]] | Day 17：`IsIntegral` / Day 18：`EnableIf` | 编译期条件分支、enable_if |
| [[auto 类型推导详解\|auto 类型推导详解]] | 通用基础 | `auto`/`auto&`/`auto&&`/`decltype(auto)` 的区别 |
| [[C++20 Concepts 与约束式泛型\|C++20 Concepts 与约束式泛型]] | Day 19：`concept` 约束 `max` | 声明式约束 vs SFINAE |
| [[运算符重载的原则与陷阱\|运算符重载的原则与陷阱]] | Day 22：`Vec2` / `Vec3` | 对称性、与内置语义一致 |
| [[Lambda 表达式与闭包\|Lambda 表达式与闭包]] | Day 23：委托系统 / Day 24：`Function` | 捕获模式、闭包类型 |
| [[std-function 的实现与开销\|std::function 的实现与开销]] | Day 24：`Function` / Day 69：事件系统 | 类型擦除、SOO、手写 Function 前置 |
| [[虚函数与多态本质\|虚函数与多态本质]] | Day 24：`Function` / Day 25：事件系统 | 类型擦除的底层机制 |
| [[成员函数指针的底层表示\|成员函数指针]] | Day 24：`Function` | 委托系统实现基础 |
| [[观察者模式与委托\|观察者模式与委托]] | Day 25：事件系统（多播委托） | Subject/Observer 生命周期、Delegate 设计 |
| [[返回值优化与 guaranteed elision\|返回值优化与 guaranteed elision]] | Day 26：`Vec3` 返回优化 | RVO/NRVO/C++17 guaranteed elision |
| [[constexpr 关键字\|constexpr 与编译期计算]] | Day 11/15/16/20：编译期编程 | 编译期计算能力 |
| [[宏编程\|宏编程与 X-Macro]] | Day 16：代码生成 | 预处理器生成枚举映射 |
...
| [[vector 的扩容策略与摊还分析\|std::vector 的扩容策略与摊还分析]] | Day 37：`Vector` | 增长因子、reallocate、摊还分析 |
| [[SoA、AoS 与 AOSOA\|SoA、AoS 与 AOSOA]] | Day 68：ComponentArray | ECS 组件存储布局 |
| [[C++11 内存模型与 happens-before\|C++11 内存模型与 happens-before]] | 第 10 周前置 | 数据竞争、happens-before |
| [[原子操作与无锁编程基础\|原子操作与无锁编程基础]] | Day 64：自旋锁 / Day 65：Mutex / Day 67：无锁队列 | std::atomic、CAS、自旋锁 |
| [[内存序：relaxed、acquire、release、seq_cst\|内存序：relaxed、acquire、release、seq_cst]] | Day 65：Mutex / Day 67：无锁队列 | release-acquire、relaxed |
| [[条件变量与虚假唤醒\|条件变量与虚假唤醒]] | Day 66：线程池 | 等待-通知、谓词循环 |
| [[异步编程模型与 future-promise\|异步编程模型与 future/promise]] | Day 66：线程池 | future/promise、std::async launch policy |
| [[工作窃取队列与线程池设计\|工作窃取队列与线程池设计]] | Day 66：线程池 | 任务队列、工作窃取、线程数选择 |
| [[thread_local 的实现与边界\|thread_local 的实现与边界]] | 第 10 周前置 | TLS 存储位置、生命周期、平台边界 |

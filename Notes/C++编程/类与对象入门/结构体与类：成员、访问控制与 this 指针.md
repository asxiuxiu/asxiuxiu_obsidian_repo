...
**陷阱 1：给每个 `private` 成员都机械地写一对 `getXxx` / `setXxx`**

把成员设为 `private` 本身没有问题，问题在于：如果外部代码其实并不需要直接读写这些字段，你却给每个字段都配了一对 getter/setter，而且这些接口里没有任何校验、副作用或不变量维护，那它们跟直接公开成员没有本质区别，只是让代码更啰嗦。

访问控制的价值在于**"封装行为"**，而不在于**"形式上藏起来"**。设计接口时应该先问"外部需要对这个对象做什么"，而不是"我内部有哪些字段需要暴露"。

只有当一个值确实是外部需要知道的抽象属性（比如 `Image::width()`），或者你需要通过接口控制读取/写入规则（校验、日志、缓存、只读等）时，才值得提供 getter/setter。否则，把字段完全隐藏起来，让外部通过更高层的成员函数与对象交互，才是真的封装。

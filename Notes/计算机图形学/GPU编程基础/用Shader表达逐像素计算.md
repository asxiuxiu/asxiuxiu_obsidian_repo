...
| 你可能关心的问题 | 本质 | 在路线中的位置 |
|---|---|---|
| 光照怎么算 | 片段着色器里的光照方程 | [[Notes/计算机图形学/Roadmap#阶段三：让像素响应光（直接光照与数据流）\|阶段三]]、[[Notes/计算机图形学/Roadmap#阶段九：让像素物理可信（PBR）\|阶段九 PBR]] |
| 深度图如何应用 | 深度纹理的生产与消费 | [[Notes/计算机图形学/Roadmap#阶段七：加工最终画面（后处理与色彩空间）\|阶段七]]、[[Notes/计算机图形学/Roadmap#阶段八：光会被挡住（阴影）\|阶段八]] |
| 粒子怎么做 | 通用计算模拟 + 透明混合排序 | [[Notes/计算机图形学/Roadmap#阶段五：单 Pass 世界的边界（混合、透明与渲染顺序）\|阶段五]]、[[Notes/计算机图形学/Roadmap#阶段十三：GPU 当计算器（Compute Shader）\|阶段十三]] |
| 水体、毛发怎么渲染 | 多 Pass 编排 + 特殊光照模型 | [[Notes/计算机图形学/Roadmap#阶段十五：高级渲染专题（菜单）\|阶段十五专题]] |
| 几百个光源怎么办 | 把光照搬到屏幕空间统一算 | [[Notes/计算机图形学/Roadmap#阶段十：一百个灯（多光源与延迟渲染）\|阶段十]] |
| Shader 多了怎么管理 | 变体、编译缓存、热重载 | [[Notes/计算机图形学/Roadmap#阶段十二：渲染架构\|阶段十二]] |

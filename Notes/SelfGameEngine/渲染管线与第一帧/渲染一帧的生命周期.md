...
3. **双缓冲（分支 B）不是必需的**。Bevy 没有双缓冲整个 World——它只在 Extract 阶段做增量拷贝。UE 也没有双缓冲整个场景——它通过 `FPrimitiveSceneProxy` 在 Game Thread 和 Render Thread 之间建立轻量代理。对于个人项目，完整的状态双缓冲内存开销和同步复杂度不值得。
...
1. **UE 的多阶段剔除证明了"没有银弹"**。UE 同时维护 CPU 剔除（传统网格）和 GPU-driven（Nanite）两条路径，因为不同内容类型适合不同策略。个人项目不应该一上来就追求 GPU-driven，而应该**先让 CPU 剔除跑通、可调试，再逐步迁移**。
...
4. **GPU-driven 是长期演进方向，不是起步需求**。AMD 的 RPS SDK、Granite 引擎、ZetaRay 都展示了 GPU-driven 的威力，但它们的实现复杂度远超个人项目初期能承受的。关键是**在架构上预留 GPU-driven 的接口**——例如 `CullingSystem` 的输出是一个 `VisibleList` Resource，无论这个列表是 CPU 生成的还是 GPU 生成的，后续的 `Queue` 阶段都不需要改变。
...
4. **排序键设计应预留足够位宽**。UE 使用 64 位排序键，Bevy 也使用多字段组合键。建议预留：高 8 位为 Phase，接下来 24 位为 PSO/Material，低 32 位为深度。这种分配在不透明阶段允许 2^24 种材质组合，在透明阶段允许 2^32 的深度精度，对个人项目完全充足。
...
**失效条件**：个人项目初期、团队规模小、对调试效率要求高于极致性能。

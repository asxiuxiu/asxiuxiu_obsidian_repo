...
> **下一步**：[[窗口与输入系统]] — 元规则确立后，立刻进入阶段 1，让代码跑起来。
...
下面的代码把这个三元组翻译成了**可以直接编译运行**的 C++ 骨架。它不依赖任何图形 API，但已经具备了一个 AI 友好引擎最核心的三个能力。
...
借鉴 `CLAUDE.md` / `AGENTS.md` 思想，把引擎的静态知识（组件列表、常见模式、最佳实践）预加载到 system prompt 中，作为**可缓存的静态前缀**。动态信息（当前选中实体、最近变更）作为**不可缓存的动态后缀**。明确区分两者，最大化 Prompt Caching 收益。
...
很多现有引擎基于 OOP 或 Manager 模式。当你想把它们重构为 AI 友好的 ECS 架构时，可以参考以下映射表：
...
| 方案 | 优点 | 缺点 | 适用场景 |
|------|------|------|----------|
| OOP | 人类直观、继承表达自然 | 深层指针树、虚函数开销、AI 难观测 | 小型单机游戏、人类主导开发 |
| ECS | 平铺状态、cache friendly、AI 友好 | 需要 mindset 转换、某些逻辑表达不直观 | 中大型项目、AI 协作开发、确定性需求强 |

> **诚实的边界**：ECS 不是"银弹"。对于高度状态机化、生命周期极短的 UI 动画，OOP 或组合模式可能比 ECS 更自然。但在游戏世界核心状态（Transform、渲染、物理、Gameplay）上，ECS 的优势压倒性。

### 选择 2：手写反射 vs 构建时代码生成

| 方案 | 优点 | 缺点 | 适用阶段 |
|------|------|------|----------|
| 手写 | 零构建依赖、立刻可用 | 维护成本高、容易遗漏 | 原型阶段（< 20 个组件） |
| 宏驱动 | 中等自动化、编译期检查 | 宏调试困难、跨平台宏行为不一致 | 早期开发 |
| 代码生成 | 完全自动化、可扩展性强 | 需要构建工具链支持 | 正式项目 |
